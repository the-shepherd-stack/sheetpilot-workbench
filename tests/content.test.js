...
test('homepage presents the tool and revenue path', () => {
  ...
  assert.match(page, /500 formula runs per month for \$9, with run history kept in this browser/);
  assert.match(page, /Stripe returns you to the workbench and unlocks access on this browser/);
  assert.match(page, /id="copy-upgrade"/);
  assert.match(page, /Copied\. Need more formulas this month/);
  assert.match(page, /500 runs per month for \$9, with run history kept in this browser/);
  ...
  assert.match(page, /Founding access is \$9 for 500 runs per month, with run history kept in this browser/);
...
test('SheetXAI alternative page keeps the comparison bounded', () => {
  ...
  assert.match(page, /Two guest tries|2 guest tries/);
  assert.match(page, /500 formula runs per month for \$9, with run history kept in this browser/);
...
test('GPT Workspace alternative page targets in-Sheets AI comparison without overclaiming', () => {
  const page = read('gpt-workspace-alternative/index.html');
  const homepage = read('index.html');
  const sitemap = read('sitemap.xml');

  assert.match(page, /GPT Workspace Alternative for One Formula Problem/);
  assert.match(page, /A GPT Workspace alternative for one formula problem/);
  assert.match(page, /AI work inside Google Sheets and Google Workspace/);
  assert.match(page, /in-Sheets AI sidebar, GPT formulas, direct insertion, or chat-with-sheet workflow/);
  assert.match(page, /direct formula insertion, GPT formulas in cells, chat with the sheet/);
  assert.match(page, /nested IF, XLOOKUP, QUERY, IMPORTRANGE, ARRAYFORMULA, and custom Apps Script functions/);
  assert.match(page, /chat with Sheets, and GPT formulas that can run inside sheet cells/);
  assert.match(page, /join 7M\+ professionals/);
  assert.match(page, /GPT Workspace details verified June 7, 2026/);
  assert.match(page, /=SUMIFS\(D2:D500,B2:B500,&quot;West&quot;,C2:C500,&quot;Paid&quot;,A2:A500,&quot;&gt;=&quot;&amp;DATE\(2026,6,1\),A2:A500,&quot;&lt;&quot;&amp;DATE\(2026,7,1\)\)/);
  assert.match(page, /2 guest tries/);
  assert.match(page, /500 runs per month/);
  assert.match(page, /What should the formula do\?|Paste the formula/);
...
test('AI Assist for Gemini alternative page targets Workspace add-on comparison without overclaiming', () => {
  const page = read('ai-assist-gemini-sheets-alternative/index.html');
  const homepage = read('index.html');
  const sitemap = read('sitemap.xml');

  assert.match(page, /AI Assist for Gemini in Sheets Alternative for One Formula Problem/);
  assert.match(page, /An AI Assist for Gemini alternative for one formula problem/);
  assert.match(page, /Google Workspace add-on for AI inside Sheets, Docs, and Forms/);
  assert.match(page, /enrichment, categorization, translation, summaries, content creation, prompt organization, and API-key-based Gemini workflows/);
  assert.match(page, /As of June 11, 2026, the listing shows a last-updated date of April 17, 2026, 315 reviews, and 6M\+ users/);
  assert.match(page, /data enrichment, refining, categorization, retrieval, translation, editing, summarization, content creation/);
  assert.match(page, /product-catalog work, review summarization, SEO metadata, ad copy, translations/);
  assert.match(page, /installed documents, forms, spreadsheets, third-party sidebar content, external-service connection/);
  assert.match(page, /AI Assist for Gemini in Sheets details checked against the Google Workspace Marketplace listing on June 11, 2026/);
  assert.match(page, /=ARRAYFORMULA\(IF\(A2:A=&quot;&quot;,&quot;&quot;,IFNA\(VLOOKUP\(A2:A,Customers!A:D,4,FALSE\),&quot;Check customer ID&quot;\)\)\)/);
  assert.match(page, /Two guest tries with no signup/);
  assert.match(page, /500 runs per month/);
  assert.match(page, /What should the formula do\?|Paste the formula/);
  assert.match(page, /Upgrade \$9/);
  assert.match(page, new RegExp(`data-checkout href="${checkoutUrl}"`));
  assert.match(homepage, /href="\/ai-assist-gemini-sheets-alternative\/">AI Assist for Gemini alternative/);
...
test('Sheet Formula alternative page targets free formula-generator comparison without overclaiming', () => {
  const page = read('sheet-formula-alternative/index.html');
  const homepage = read('index.html');
  const sitemap = read('sitemap.xml');

  assert.match(page, /Sheet Formula Alternative for One Formula Problem/);
  assert.match(page, /A Sheet Formula alternative for one formula problem/);
  assert.match(page, /free Google Sheets formula generator with a simple prompt box/);
  assert.match(page, /usage limits, sign-in extras, and separate Google Sheets add-ons sold alongside it/);
  assert.match(page, /signing in unlocks extras such as AI formula analysis/);
  assert.match(page, /Sheet Automation, Sheet Notification, and Share Single Sheet/);
  assert.match(page, /Sheet Formula details verified June 11, 2026/);
  assert.match(page, /=SUMIFS\(D2:D500,B2:B500,&quot;West&quot;,C2:C500,&quot;Paid&quot;,A2:A500,&quot;&gt;=&quot;&amp;DATE\(2026,6,1\),A2:A500,&quot;&lt;&quot;&amp;DATE\(2026,7,1\)\)/);
  assert.match(page, /2 guest tries/);
  assert.match(page, /500 runs per month/);
  assert.match(page, /What should the formula do\?|Paste the formula/);
...
test('excel MAKEARRAY not working page targets generated array repair intent without overclaiming', () => {
  const page = read('excel-makearray-not-working/index.html');
  const homepage = read('index.html');
  const sitemap = read('sitemap.xml');

  assert.match(page, /Excel MAKEARRAY Not Working - Fix #VALUE, #SPILL, #NAME/);
  assert.match(page, /Repair a MAKEARRAY formula that is generating the wrong grid/);
  assert.match(page, /MAKEARRAY builds a calculated array from a row count, column count, and LAMBDA\(row, col, calculation\)/);
  assert.match(page, /rows or columns are below 1, non-numeric, or reversed/);
  assert.match(page, /#VALUE! Incorrect Parameters from a LAMBDA argument mismatch/);
  assert.match(page, /#VALUE! from a row or column argument below 1 or non-numeric/);
  assert.match(page, /row and column indexes are used in the wrong order/);
  assert.match(page, /Make the LAMBDA accept exactly two parameters/);
  assert.match(page, /row 2 column 3 should be 6/);
  assert.match(page, /row-versus-column index use/);
  assert.match(page, /=MAKEARRAY\(5,3,LAMBDA\(r,c,r\*c\)\)/);
  assert.match(page, /What should the formula do\?|Paste the formula/);
  assert.match(page, /Upgrade \$9/);
  assert.match(page, new RegExp(`data-checkout href="${checkoutUrl}"`));
  assert.match(homepage, /href="\/excel-makearray-not-working\/">Excel MAKEARRAY repair/);
  assert.match(sitemap, /https:\/\/writemyformula\.com\/excel-makearray-not-working\//);
...
test('FormulaPilot alternative page targets focused formula comparison without overclaiming', () => {
  ...
  assert.match(page, /formula library, checker, or reference hub/);
  assert.match(page, /write one formula, repair one broken formula, explain one inherited formula/);
